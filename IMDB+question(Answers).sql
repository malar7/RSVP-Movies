...
WHERE g.genre = (
    SELECT g2.genre
    FROM genre AS g2
    INNER JOIN movie AS m2 ON g2.movie_id = m2.id
    GROUP BY g2.genre
    ORDER BY COUNT(g2.genre) DESC
    LIMIT 1
...
GROUP BY
    name
HAVING
    COUNT(CASE WHEN m.country = 'India' THEN 1 ELSE NULL END) >= 3
LIMIT 5;
...
SELECT
    ...
    AVG(avg_rating) AS avg_rating, -- Wrap avg_rating in an aggregate function
